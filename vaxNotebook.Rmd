...
comp_selangor <- data.frame(date = vax_selangor$date, data = c(vax_selangor$daily, cases_selangor$daily))
#View(comp_selangor)

ggplot(comp_selangor, aes(x = as.Date(date), y = data , group = 2, color = data)) + geom_line() + labs(title = "Daily New Cases in Selangor", x = "Date", y = "Total Daily Dose", color = "State") + theme_fivethirtyeight() + theme(axis.title = element_text()) + scale_x_date(date_breaks = "1 month", labels = date_format("%d-%b"))
```
