...
pop_selangor <- population_states[population_states$state == "Selangor",]
pop_kl <- population_states[population_states$state == "W.P. Kuala Lumpur",]
pop_putra <- population_states[population_states$state == "W.P. Putrajaya",]
...
ggplot(val, aes(state, full)) + theme_bw() + geom_bar(stat = "identity", position = position_dodge()) + geom_text(aes(label = val_y), vjust = -0.3, position = position_dodge(0.9), size = 4) + labs(x = "Status", y = "Percentage (%)", title = "Vaccinated Status in Malaysia") + ylim(0, 100)
```
